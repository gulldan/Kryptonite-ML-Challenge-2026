...
- Every model run, validation run, leaderboard submission candidate, reranking/postprocessing
  run, and hypothesis check must be recorded during the same work session in two places:
  one summary row in the top-level `Leaderboard History` table in
  `docs/challenge-experiment-history.md`, and one detailed Markdown trail at
  `docs/trails/<slug>.md`.
- Keep `docs/challenge-experiment-history.md` as the compact leaderboard table only. Do
  not append long narrative experiment sections there; put those details in
  `docs/trails/<slug>.md`.
...
- Important experiment changes and outcomes must be recorded in both the table and the
  relevant trail file, especially anything used for a leaderboard submission,
  presentation, or strategic decision.
...
- For this hackathon, updating `docs/challenge-experiment-history.md` is mandatory, not
  ...
  `docs/trails/<slug>.md` experiment trail and the top-level `Leaderboard History` table
  with the exact score, submission artifact path, validator status, and decision.
- After changing public leaderboard rows, regenerate the README chart with
  `uv run python scripts/render_public_lb_chart.py`, commit the updated
  `docs/assets/public-lb-score.svg`, and keep the image embedded in `README.md` so the
  jury immediately sees how the hypotheses evolved.
- When a hypothesis is run on `remote`, record the remote run id, log path, checkpoint path,
...
- For GPU-bound work, connect over `<remote-shell>` to `<remote-environment>`
- On `<remote-environment>`, run repository commands from `<repo-root>`
- On both local machines and `<remote-environment>`, the canonical working environment is `<repo-root>/.venv` (or `./.venv` locally) created by `uv sync`
- Keep local datasets on the GPU machine in `<repo-root>/datasets`
- Preferred workflow is: commit locally, push, then fetch/pull on `<remote-environment>` before running GPU jobs
- Do not try to version datasets from `datasets/` or make git depend on them being present

### H100 Remote Workflow

The H100 remote target is the current high-memory GPU execution target for this challenge. Use it for
...
- Keep the local repository and the remote repository synchronized before running remote
...
- Do not rely on `artifacts/` being included in the general repo sync. Copy only the
  ...
  current experiment.
- After editing `AGENTS.md`, `docs/challenge-experiment-history.md`, `docs/trails/`,
  README chart artifacts, configs, scripts, or `src/` code needed by a remote job, sync
  those files to the remote target before launching.
...
```bash
<remote-shell> <remote-host> 'docker exec container bash -lc "cd <repo-root> && uv sync --dev --group train"'
```
...
- Use `CUDA_VISIBLE_DEVICES=0` and `CUDA_VISIBLE_DEVICES=1` for independent hypotheses on
  the two H100 GPUs. The current training/inference pipeline is not assumed to be DDP-safe
...
```bash
<remote-shell> <remote-host> 'docker exec container bash -lc "cd <repo-root> && tail -n 80 artifacts/logs/<run>.log && nvidia-smi"'
```

Experiment documentation for remote runs:

- Every remote launch must be reflected in the same session by adding/updating a
  `Leaderboard History` table row in `docs/challenge-experiment-history.md` and a detailed
  `docs/trails/<slug>.md` record: hypothesis, command/config, GPU assignment, remote log
  path, artifact paths, local metrics, public LB score when available, and decision.
- Public LB results from files generated on the remote target must be added to the top-level
  `Leaderboard History` table immediately after the score is known.
- Sync `docs/challenge-experiment-history.md`, the relevant `docs/trails/<slug>.md`, and
  README chart updates back to the remote target after updating them locally, so the remote workspace
  remains self-describing.
- For training jobs on the remote target, the history entry must include enough parameters to rerun
...
- After merging completed work into `develop`, push `develop` and use that updated branch as the base for subsequent integration work on `<remote-environment>`
