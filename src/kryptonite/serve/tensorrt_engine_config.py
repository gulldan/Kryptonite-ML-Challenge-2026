...
@dataclass(frozen=True, slots=True)
class TensorRTFP16ProfileConfig:
    profile_id: str
...
class TensorRTFP16BuildConfig:
    ...
    profiles: tuple[TensorRTFP16ProfileConfig, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if not self.profiles:
            raise ValueError("build.profiles must contain at least one profile.")
        profile_ids = [profile.profile_id for profile in self.profiles]
        if len(set(profile_ids)) != len(profile_ids):
            raise ValueError("build.profiles profile_id values must be unique.")
    ...
    @property
    def opt_batch_size(self) -> int:
        return max(profile.opt_batch_size for profile in self.profiles)
    ...
    @property
    def opt_frame_count(self) -> int:
        return max(profile.opt_frame_count for profile in self.profiles)
...
class TensorRTFP16Config:
    ...
    def __post_init__(self) -> None:
        ...
        for sample in self.evaluation.samples:
            if not any(
                profile.covers(batch_size=sample.batch_size, frame_count=sample.frame_count)
...
def load_tensorrt_fp16_config(*, config_path: Path | str) -> TensorRTFP16Config:
    ...
    config = TensorRTFP16Config(
        ...
        build=TensorRTFP16BuildConfig(
            ...
            profiles=_parse_profiles(build),
            builder_optimization_level=_coerce_non_negative_int(
                build.get("builder_optimization_level", 3),
                field_name="build.builder_optimization_level",
            ),
...
def _parse_profiles(build: dict[str, Any]) -> tuple[TensorRTFP16ProfileConfig, ...]:
    raw_profiles = build.get("profiles")
    if raw_profiles is None:
        return (
            TensorRTFP16ProfileConfig(
                profile_id="default",
    ...
    profiles: list[TensorRTFP16ProfileConfig] = []
    for index, raw_profile in enumerate(raw_profiles):
        profile = _coerce_mapping(raw_profile, field_name=f"build.profiles[{index}]")
        profiles.append(
            TensorRTFP16ProfileConfig(
