...
class ManifestRow:
    ...
    def from_mapping(
        ...
        *,
        ...
        return cls(
            ...
            extra_fields=_extract_extra_fields(entry),
    ...
    def to_dict(self, *, extra_fields: Mapping[str, object] | None = None) -> dict[str, object]:
        ...
        conflicting_extra_keys = sorted(set(payload) & set(self.extra_fields))
        if conflicting_extra_keys:
            joined = ", ".join(conflicting_extra_keys)
            raise ValueError(f"Stored extra fields conflict with canonical fields: {joined}")
...
def _extract_extra_fields(entry: Mapping[str, object]) -> dict[str, object]:
    return {
        str(key): value
        for key, value in entry.items()
        if str(key) not in _CANONICAL_MANIFEST_FIELDS
    }
