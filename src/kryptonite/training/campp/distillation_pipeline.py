"""Runnable CAM++ student distillation built on top of the stage-3 recipe."""
...
from kryptonite.training.teacher_peft.model import build_feature_batch

from ..augmentation_runtime import TrainingAugmentationRuntime
from .distillation_config import CAMPPlusDistillationConfig
...
REPORT_FILE_NAME = "campp_distillation_report.md"
DISTILLATION_SUMMARY_FILE_NAME = "distillation_summary.json"
DISTILLATION_SCHEDULE_FILE_NAME = "distillation_schedule.json"
...
class BaselineComparisonSummary:
    ...
    distilled_report_path: str
    baseline_score_summary_path: str
    distilled_score_summary_path: str
    baseline_eer: float
    distilled_eer: float
    eer_delta: float
    baseline_min_dcf: float
    distilled_min_dcf: float
    min_dcf_delta: float
    baseline_score_gap: float | None
    distilled_score_gap: float | None
...
@dataclass(frozen=True, slots=True)
class CAMPPlusDistillationRunArtifacts:
    output_root: str
    checkpoint_path: str
    training_summary_path: str
    distillation_summary_path: str
    embeddings_path: str
    ...
    distillation_epochs: tuple[DistillationEpochBreakdown, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "distillation_summary_path": self.distillation_summary_path,
            ...
            "distillation_epochs": [epoch.to_dict() for epoch in self.distillation_epochs],
...
def run_campp_distillation(
    config: CAMPPlusDistillationConfig,
    *,
    config_path: Path | str,
    device_override: str | None = None,
    teacher_loader: Callable[
    ...
    validate_training_precision(
        ...
        baseline_name="CAM++ distillation",
    )
    if config.project.training.max_epochs <= 0:
        raise ValueError("training.max_epochs must be positive for CAM++ distillation runs.")
    ...
    if config.project.tracking.enabled:
        ...
        tracker_run = tracker.start_run(kind="campp-distillation", config=config.to_dict())
    ...
    teacher_checkpoint_dir, teacher_metadata, teacher_feature_extractor, teacher_encoder = (
        teacher_loader(
    ...
    distillation_epochs: list[DistillationEpochBreakdown] = []
    ...
    for epoch in range(config.project.training.max_epochs):
        ...
        if phase_index != current_phase_index:
            ...
            dataset, sampler, loader = build_distillation_dataloader(
                rows=train_rows,
        ...
        metrics = run_distillation_batches(
            model=model,
            classifier=classifier,
            criterion=criterion,
            ...
            classification_weight=config.distillation.classification_weight,
            embedding_weight=config.distillation.embedding_weight,
            score_weight=config.distillation.score_weight,
            teacher_batch_builder=build_feature_batch,
        )
        if metrics.total_examples <= 0:
            raise ValueError("Distillation loader produced zero examples.")
        epoch_summary = DistillationEpochBreakdown(
            epoch=epoch + 1,
            mean_loss=round(metrics.total_loss / metrics.total_examples, 6),
            mean_classification_loss=round(
                metrics.total_classification_loss / metrics.total_examples,
                6,
            ),
            mean_embedding_loss=round(metrics.total_embedding_loss / metrics.total_examples, 6),
            mean_score_loss=round(metrics.total_score_loss / metrics.total_examples, 6),
            accuracy=round(metrics.total_correct / metrics.total_examples, 6),
            learning_rate=round(training_runtime.current_learning_rate(), 8),
        )
        distillation_epochs.append(epoch_summary)
            ...
            tracker_run.log_metrics(
                {
                    ...
                    "train_accuracy": epoch_summary.accuracy,
                    "classification_loss": epoch_summary.mean_classification_loss,
    ...
    training_summary = TrainingSummary(
        ...
        epochs=tuple(
            ...
            for epoch in distillation_epochs
        ),
    )
    ...
    distillation_summary_path = output_root / DISTILLATION_SUMMARY_FILE_NAME
    distillation_summary_path.write_text(
        json.dumps(
            {
                ...
                "distillation": config.distillation.to_dict(),
                "epochs": [epoch.to_dict() for epoch in distillation_epochs],
            },
            indent=2,
            sort_keys=True,
        ),
    ...
    (output_root / DISTILLATION_SCHEDULE_FILE_NAME).write_text(
        json.dumps(
            {
                "student_checkpoint": config.student.checkpoint,
                "teacher_checkpoint": str(teacher_checkpoint_dir),
    ...
    embedding_summary, metadata_rows = export_dev_embeddings(
        ...
        device=device,
        embedding_source="campp_distillation",
    )
    ...
    comparison = _build_comparison_summary(
        ...
        distilled_checkpoint_path=str(checkpoint_path),
        baseline_score_summary=baseline_score_summary,
        distilled_score_summary=score_summary,
        baseline_verification_report=baseline_verification_report,
        distilled_verification_report=verification_report,
    )
    ...
    if tracker_run is not None:
        final_epoch = distillation_epochs[-1]
        tracker_run.log_metrics(
            {
                "train_loss": final_epoch.mean_loss,
                "train_accuracy": final_epoch.accuracy,
                "classification_loss": final_epoch.mean_classification_loss,
        ...
        for artifact_path in (
            ...
            output_root / DISTILLATION_SCHEDULE_FILE_NAME,
    ...
    return CAMPPlusDistillationRunArtifacts(
        output_root=str(output_root),
        checkpoint_path=str(checkpoint_path),
        training_summary_path=str(training_summary_path),
        distillation_summary_path=str(distillation_summary_path),
        ...
        training_summary=training_summary,
        distillation_epochs=tuple(distillation_epochs),
        embedding_summary=embedding_summary,
        score_summary=score_summary,
        verification_report=verification_report,
        comparison=comparison,
...
def _evaluate_reference_checkpoint(
    *,
    ...
    _, _, checkpoint_model = load_campp_encoder_from_checkpoint(
        torch=torch,
    ...
    embedding_summary, metadata_rows = export_dev_embeddings(
        ...
        chunking=chunking,
        device=device,
        embedding_source="campp_reference",
    )
    score_summary = score_trials(
        output_root=output_root,
        trials_path=trials_path,
...
def _build_comparison_summary(
    *,
    ...
    distilled_score_summary: ScoreSummary,
    baseline_verification_report: Any,
    distilled_verification_report: Any,
) -> BaselineComparisonSummary:
    baseline_metrics = baseline_verification_report.summary.metrics
    distilled_metrics = distilled_verification_report.summary.metrics
    baseline_score_gap = baseline_score_summary.score_gap
    distilled_score_gap = distilled_score_summary.score_gap
    score_gap_delta = None
    if baseline_score_gap is not None and distilled_score_gap is not None:
        score_gap_delta = round(distilled_score_gap - baseline_score_gap, 6)
    return BaselineComparisonSummary(
        ...
        distilled_report_path=distilled_verification_report.report_markdown_path,
        baseline_score_summary_path=baseline_score_summary.scores_path,
        distilled_score_summary_path=distilled_score_summary.scores_path,
        baseline_eer=baseline_metrics.eer,
        distilled_eer=distilled_metrics.eer,
        eer_delta=round(distilled_metrics.eer - baseline_metrics.eer, 6),
        baseline_min_dcf=baseline_metrics.min_dcf,
        distilled_min_dcf=distilled_metrics.min_dcf,
        min_dcf_delta=round(distilled_metrics.min_dcf - baseline_metrics.min_dcf, 6),
        baseline_score_gap=baseline_score_gap,
        distilled_score_gap=distilled_score_gap,
...
def _render_comparison_markdown(
    *,
    ...
    distilled_report = relative_to_project(
        Path(comparison.distilled_report_path),
        project_root=project_root,
    )
    lines = [
        "# CAM++ Distillation Baseline Comparison",
        "",
        f"- Baseline checkpoint: `{comparison.baseline_checkpoint_path}`",
        f"- Distilled checkpoint: `{comparison.distilled_checkpoint_path}`",
        f"- Baseline report: `{baseline_report}`",
        f"- Distilled report: `{distilled_report}`",
        "",
        "| Metric | Baseline | Distilled | Delta (distilled - baseline) |",
        "| --- | --- | --- | --- |",
        (
            f"| EER | `{comparison.baseline_eer}` | `{comparison.distilled_eer}` | "
            f"`{comparison.eer_delta}` |"
        ),
        (
            f"| MinDCF | `{comparison.baseline_min_dcf}` | "
            f"`{comparison.distilled_min_dcf}` | `{comparison.min_dcf_delta}` |"
        ),
        (
            f"| Score gap | `{comparison.baseline_score_gap}` | "
            f"`{comparison.distilled_score_gap}` | `{comparison.score_gap_delta}` |"
        ),
        "",
    ]
    return "\n".join(lines)
...
def _render_distillation_appendix(
    *,
    config: CAMPPlusDistillationConfig,
    ...
    teacher_checkpoint = relative_to_project(
    ...
    lines = [
        ...
        f"- Student warm start: `{config.student.checkpoint}`",
        f"- Baseline comparison checkpoint: `{config.student.resolved_comparison_checkpoint}`",
        f"- Teacher checkpoint: `{teacher_checkpoint}`",
        (
            "- Loss weights: "
            f"classification={config.distillation.classification_weight}, "
            f"embedding={config.distillation.embedding_weight}, "
            f"score={config.distillation.score_weight}"
        ),
        "",
        "## Distillation Epochs",
        "",
        "| Epoch | Total | Classification | Embedding | Score | Accuracy | LR |",
        "| --- | --- | --- | --- | --- | --- | --- |",
    ]
    for epoch in distillation_epochs:
        lines.append(
            ...
            f"{epoch.epoch} | "
            f"{epoch.mean_loss} | "
            f"{epoch.mean_classification_loss} | "
    ...
    lines.extend(
        [
            ...
            f"- EER delta (distilled - baseline): `{comparison.eer_delta}`",
            f"- MinDCF delta (distilled - baseline): `{comparison.min_dcf_delta}`",
            f"- Score-gap delta (distilled - baseline): `{comparison.score_gap_delta}`",
            "",
        ]
    )
    return "\n".join(lines)
