...
CORE_PACKAGE_SPECS: tuple[PackageSpec, ...] = (
    ...
    PackageSpec(group="train", distribution="peft", module="peft", probe="distribution"),
...
def _probe_package(spec: PackageSpec) -> PackageProbe:
    if spec.probe == "distribution":
        ...
        return PackageProbe(
            ...
            available=distribution_version is not None,
            version=distribution_version,
            details={"probe": "distribution"},
            error=(
                None
                if distribution_version is not None
                else f"PackageNotFoundError: {spec.distribution} is not installed"
            ),
        )
