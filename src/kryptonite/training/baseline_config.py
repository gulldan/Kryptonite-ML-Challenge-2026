...
class BaselineObjectiveConfig:
    ...
    pseudo_label_smoothing: float = 0.0
    pseudo_loss_weight: float = 1.0
    pseudo_gll_enabled: bool = False
    pseudo_gll_threshold_start: float = 0.0
    pseudo_gll_threshold_end: float = 0.0
    pseudo_gll_verified_always: bool = True
    inter_topk_penalty_weight: float = 0.0
    inter_topk_penalty_k: int = 0
    inter_topk_margin: float = 0.0
    ...
    def __post_init__(self) -> None:
        ...
        if self.subcenters_per_class <= 0:
        ...
        for field_name, value in (
            ...
            ("pseudo_label_smoothing", self.pseudo_label_smoothing),
        ):
            if not 0.0 <= value < 1.0:
                raise ValueError(f"{field_name} must be within [0.0, 1.0)")
        if self.pseudo_loss_weight < 0.0:
            raise ValueError("pseudo_loss_weight must be non-negative")
        if self.inter_topk_penalty_weight < 0.0:
            raise ValueError("inter_topk_penalty_weight must be non-negative")
        if self.inter_topk_penalty_k < 0:
            raise ValueError("inter_topk_penalty_k must be non-negative")
        if self.inter_topk_margin < 0.0:
            raise ValueError("inter_topk_margin must be non-negative")
        for field_name, value in (
            ("pseudo_gll_threshold_start", self.pseudo_gll_threshold_start),
            ("pseudo_gll_threshold_end", self.pseudo_gll_threshold_end),
        ):
            if not 0.0 <= value <= 1.0:
                raise ValueError(f"{field_name} must be within [0.0, 1.0]")
