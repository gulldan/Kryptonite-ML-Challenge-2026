...
def _compute_classification_loss(
    *,
    ...
    pseudo_confidence_threshold: float | None,
) -> torch.Tensor:
    if objective_config is None:
        return criterion(logits, labels)

    pseudo_mask = _pseudo_mask(batch=batch, device=device)
    if not bool(pseudo_mask.any().item()):
        per_sample_loss = criterion(
            logits,
            labels,
            label_smoothing=objective_config.label_smoothing,
            reduction="none",
        )
        per_sample_loss = per_sample_loss + _inter_topk_penalty(
            logits=logits,
            labels=labels,
            objective_config=objective_config,
        )
    ...
    pseudo_loss = criterion(
        logits,
        labels,
        label_smoothing=objective_config.pseudo_label_smoothing,
        reduction="none",
    )
    per_sample_loss = torch.where(pseudo_mask, pseudo_loss, base_loss)
    per_sample_loss = per_sample_loss + _inter_topk_penalty(
        logits=logits,
        labels=labels,
        objective_config=objective_config,
    ...
    sample_weights = batch.sample_weights.to(device=device, dtype=per_sample_loss.dtype)
    pseudo_loss_multiplier = torch.full_like(sample_weights, objective_config.pseudo_loss_weight)
    sample_weights = torch.where(
        pseudo_mask, sample_weights * pseudo_loss_multiplier, sample_weights
    )

    if objective_config.pseudo_gll_enabled:
        threshold = 0.0 if pseudo_confidence_threshold is None else pseudo_confidence_threshold
        assigned_confidence = (
        ...
        keep_pseudo = assigned_confidence >= threshold
        if objective_config.pseudo_gll_verified_always:
            verified = batch.pseudo_verified_mask.to(device=device)
            keep_pseudo = keep_pseudo | verified
        sample_weights = torch.where(
            pseudo_mask & ~keep_pseudo,
            torch.zeros_like(sample_weights),
            sample_weights,
        )

    weighted_loss = per_sample_loss * sample_weights
    return weighted_loss.sum() / sample_weights.sum().clamp_min(1.0)


def _inter_topk_penalty(
    *,
    logits: torch.Tensor,
    labels: torch.Tensor,
    objective_config: BaselineObjectiveConfig,
) -> torch.Tensor:
    if (
        objective_config.inter_topk_penalty_weight <= 0.0
        or objective_config.inter_topk_penalty_k <= 0
        or logits.shape[1] <= 1
    ):
        return torch.zeros_like(labels, dtype=logits.dtype)

    top_k = min(objective_config.inter_topk_penalty_k, logits.shape[1] - 1)
    ...
    impostor_logits = logits.masked_fill(target_mask, -torch.inf)
    top_impostors = impostor_logits.topk(top_k, dim=1).values
    penalty = torch_functional.softplus(
        (top_impostors + objective_config.inter_topk_margin - target_logits)
        * objective_config.scale
    ).mean(dim=1)
    return penalty * objective_config.inter_topk_penalty_weight


def _pseudo_mask(*, batch: TrainingBatch, device: torch.device) -> torch.Tensor:
    values = [
        split == "pseudo_train" or source_dataset.startswith("test_public_")
        for split, source_dataset in zip(batch.splits, batch.source_datasets, strict=True)
    ]
