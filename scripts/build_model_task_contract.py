"""Build a reproducible task and model contract snapshot."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to the TOML config that anchors the contract snapshot.",
    )
    parser.add_argument(
        "--output-root",
        default=DEFAULT_MODEL_TASK_CONTRACT_OUTPUT_ROOT,
        help="Directory where the generated contract snapshot should be written.",
    )
    parser.add_argument(
        "--output",
        choices=("text", "json"),
...
def main() -> None:
    ...
    contract = build_model_task_contract(config, output_root=args.output_root)
    written = write_model_task_contract(contract)
