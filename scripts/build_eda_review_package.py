...
def main() -> None:
    ...
    file_stats = _with_review_columns(
        pl.read_parquet(audio_dir / "file_stats.parquet"),
        domain_clusters_path=audio_dir / "domain_clusters.csv",
    )
    ...
    write_review_plots(
        ...
        audio_dir=audio_dir,
        public_dir=public_dir,
        local_manifest_path=audio_dir / "val_manifest.csv",
    )
...
def _with_review_columns(frame: pl.DataFrame, *, domain_clusters_path: Path) -> pl.DataFrame:
    domain = pl.read_csv(domain_clusters_path).select("filepath", "domain_cluster")
    return (
