...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--stage1-config",
        default="configs/training/w2vbert2-mfa-lora-stage1.toml",
    )
    parser.add_argument(
        "--stage2-config",
        default="configs/training/w2vbert2-mfa-joint-ft-stage2.toml",
    )
    parser.add_argument(
        "--stage3-config",
        default="configs/training/w2vbert2-mfa-lmft-stage3.toml",
    )
    parser.add_argument(
        "--report-json",
...
def _run_stage2(args: argparse.Namespace, *, stage1_checkpoint: str) -> dict[str, Any]:
    payload = _run_json(
        [
            sys.executable,
            "scripts/run_teacher_peft_finetune.py",
            "--config",
            args.stage2_config,
            "--init-checkpoint",
            stage1_checkpoint,
