...
def test_balanced_speaker_batch_sampler_can_balance_external_domain() -> None:
    rows = [
        _manifest_row("speaker_alpha", "alpha_0.wav", source_dataset="kryptonite_participants"),
        _manifest_row("speaker_bravo", "bravo_0.wav", source_dataset="kryptonite_participants"),
        _manifest_row("speaker_charlie", "charlie_0.wav", source_dataset="kryptonite_participants"),
        _manifest_row("cnceleb_delta", "delta_0.wav", source_dataset="cnceleb_v2_openslr82"),
        _manifest_row("cnceleb_echo", "echo_0.wav", source_dataset="cnceleb_v2_openslr82"),
        _manifest_row("cnceleb_foxtrot", "foxtrot_0.wav", source_dataset="cnceleb_v2_openslr82"),
    ]
    sampler = BalancedSpeakerBatchSampler(
        rows=rows,
        batch_size=4,
        ...
        domain_balance_external_share=0.25,
    )
    sampler.set_epoch(0)

    first_batch = next(iter(sampler))
    external_count = sum(
        rows[item.row_index].source_dataset.startswith("cnceleb") for item in first_batch
    )
