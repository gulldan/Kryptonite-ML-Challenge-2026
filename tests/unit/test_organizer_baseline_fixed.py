...
def _install_dataset_stubs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fake_torchaudio.load = lambda _path: (_ for _ in ()).throw(
        RuntimeError("torchaudio.load is not used by these tests")
    )
    fake_torchaudio.transforms = types.SimpleNamespace(Resample=object)
...
def _install_train_stubs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fake_torch.cat = lambda *_args, **_kwargs: None
    fake_torch.manual_seed = lambda _seed: None
    fake_torch.no_grad = lambda: lambda func: func
    ...
    fake_torch.Generator = lambda: types.SimpleNamespace(manual_seed=lambda _seed: None)
    fake_torch.optim = types.SimpleNamespace(Optimizer=object, AdamW=object)
    fake_torch.nn = types.SimpleNamespace(Module=object, CrossEntropyLoss=lambda: None)
    ...
    fake_metrics.precision_at_k = lambda *_args, **_kwargs: {}
    fake_dataset = types.ModuleType("src.dataset")
    fake_dataset.SpeakerDataset = object
    fake_model = types.ModuleType("src.model")
    fake_model.ECAPASpeakerId = object
    fake_tqdm = types.ModuleType("tqdm")
    fake_tqdm.tqdm = lambda iterable, *_args, **_kwargs: iterable
...
def _install_calc_metric_stubs(monkeypatch: pytest.MonkeyPatch) -> None:
    fake_src = types.ModuleType("src")
    fake_metrics = types.ModuleType("src.metrics")
    fake_metrics.precision_at_k_from_indices = lambda *_args, **_kwargs: {"precision@10": 1.0}
    monkeypatch.setitem(sys.modules, "src", fake_src)
    monkeypatch.setitem(sys.modules, "src.metrics", fake_metrics)
