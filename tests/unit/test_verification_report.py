...
def test_build_verification_evaluation_report_produces_curves_and_histograms(
    tmp_path: Path,
) -> None:
...
def test_write_verification_evaluation_report_writes_current_artifacts(tmp_path: Path) -> None:
    score_rows = [
...
def test_render_verification_evaluation_markdown_includes_current_optional_inputs(
    tmp_path: Path,
) -> None:
    report = build_verification_evaluation_report(
        [
