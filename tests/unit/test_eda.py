...
def test_participant_split_csvs_convert_to_training_manifests(tmp_path) -> None:
    ...
    result = build_participant_training_manifests(
        ...
        audio_root=tmp_path / "datasets" / "participants",
    ...
    assert train_row["audio_path"] == str(
        (tmp_path / "datasets" / "participants" / "train/speaker_a/00000.flac").as_posix()
    )
...
def test_graph_tail_diagnostics_exposes_label_confidence_features() -> None:
    indices = np.array(
        [
            [1, 2, 3, 4],
            [0, 2, 3, 4],
    ...
    assert diagnostics.margin_top1_top10[0] == pytest.approx(0.1, abs=1e-6)
    assert diagnostics.reciprocal_support[0] == 2
    assert diagnostics.label_usable[0]
    assert diagnostics.same_label_candidates[0] >= 1


def test_rowwise_tail_router_uses_query_confidence_gates() -> None:
    candidates = {
        "full_c4": np.array([[1, 2], [2, 3]], dtype=np.int64),
        "exact": np.array([[1, 3], [0, 3]], dtype=np.int64),
        "weak_c4": np.array([[1, 2], [0, 2]], dtype=np.int64),
        "reciprocal_only": np.array([[2, 1], [3, 2]], dtype=np.int64),
        "soup_c4": np.array([[1, 2], [2, 3]], dtype=np.int64),
        "classaware_c4": np.array([[2, 1], [2, 3]], dtype=np.int64),
    }
    diagnostics = GraphTailDiagnostics(
        margin_top1_top10=np.array([0.08, 0.002], dtype=np.float32),
        reciprocal_support=np.array([3, 0], dtype=np.int16),
        labels=np.array([0, 1], dtype=np.int64),
        label_sizes=np.array([8, 160], dtype=np.int64),
        row_label_size=np.array([8, 160], dtype=np.int32),
        same_label_candidates=np.array([6, 0], dtype=np.int16),
        label_usable=np.array([True, False]),
        label_confidence=np.array([0.85, 0.0], dtype=np.float32),
    )

    routed, policy, details = route_tail_policies(
        candidates=candidates,
        diagnostics=diagnostics,
        config=RowwiseTailRouterConfig(
            default_policy="classaware_c4",
            min_consensus_overlap=1,
            strong_consensus_overlap=1,
            low_class_entropy=0.4,
            high_class_entropy=0.8,
            extreme_label_max_size=120,
        ),
        class_entropy=np.array([0.1, 0.9], dtype=np.float32),
    )

    assert routed[0].tolist() == [2, 1]
    assert policy[0] == "classaware_c4"
    assert routed[1].tolist() == [0, 3]
    assert policy[1] == "exact"
    assert details["selected_reason"].tolist() == [
        "low_entropy_classaware_agreement",
        "extreme_low_graph_confidence_exact",
    ]


def test_normalized_entropy_is_low_for_peaked_distribution() -> None:
    entropy = normalized_entropy(
        np.array([[0.98, 0.01, 0.01], [1.0 / 3, 1.0 / 3, 1.0 / 3]], dtype=np.float32)
    )

    assert entropy[0] < 0.15
    assert entropy[1] == pytest.approx(1.0, abs=1e-6)
