...
def test_probe_package_supports_distribution_only_mode(monkeypatch) -> None:
    monkeypatch.setattr(training_environment, "_distribution_version", lambda _: "0.18.1")

    probe = training_environment._probe_package(
        training_environment.PackageSpec(
            group="train",
            distribution="peft",
            module="peft",
            probe="distribution",
    ...
    assert probe.version == "0.18.1"
    assert probe.details == {"probe": "distribution"}
