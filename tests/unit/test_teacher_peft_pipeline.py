...
def test_teacher_peft_smoke_run_writes_checkpoint_embeddings_and_scores(
    tmp_path: Path,
    capsys,
) -> None:
    ...
    assert "[teacher-peft] init " in stderr
    assert "[train] epoch 1/1 start" in stderr
    assert "[train] epoch 1/1 batch 1/" in stderr
    assert "[teacher-peft] phase=embedding-export start" in stderr
    assert "[embed] utterance 1/4 " in stderr
