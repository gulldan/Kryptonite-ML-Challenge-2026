...
Каждая архитектура -- тонкая обёртка: создаёт encoder и делегирует общий запуск.
...
### 2. Загрузчик конфига -- `src/kryptonite/training/<name>/config.py`
...
### 3. Обёртка пайплайна -- `src/kryptonite/training/<name>/pipeline.py`
...
### 4. TOML-конфиг -- `configs/training/my-model-baseline.toml`
