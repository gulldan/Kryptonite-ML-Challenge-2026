...
from kryptonite.runtime.tensorrt_generic import MultiInputTensorRTEngineRunner
...
def main() -> None:
    ...
    widths = _resolve_retrieval_widths(args=args, row_count=manifest.height)
    print(
        f"[teacher-peft-tail] start experiment={args.experiment_id} "
        f"checkpoint={args.checkpoint_path} rows={manifest.height} "
        f"output_top_k={args.output_top_k} top_cache_k={widths['top_cache_k']} "
...
def _build_encoder_backend(args: argparse.Namespace) -> tuple[Any, Any]:
    ...
    if args.encoder_backend == "tensorrt":
        ...
        try:
            ...
            print(
                "[teacher-peft-tail] TensorRT engine load failed; "
                f"falling back to torch backend: {exc}",
                flush=True,
            )
            token = os.environ.get(args.hf_token_env) or None
            _, _, _, torch_encoder = load_teacher_peft_encoder_from_checkpoint(
                checkpoint_path=args.checkpoint_path,
                token=token,
                trainable=False,
            )
            torch_encoder = torch_encoder.to(args.device)
            torch_encoder.eval()
            return feature_extractor, _TorchTeacherPeftEncoder(torch_encoder)
...
def _resolve_retrieval_widths(*, args: argparse.Namespace, row_count: int) -> dict[str, int]:
    ...
    edge_top = min(args.edge_top, max_neighbours)
    reciprocal_top = min(args.reciprocal_top, max_neighbours)
    rank_top = min(max(args.rank_top, args.output_top_k), max_neighbours)
    shared_top = min(args.shared_top, max_neighbours)
    top_cache_k = min(
        max(args.top_cache_k, args.output_top_k, edge_top, reciprocal_top, rank_top, shared_top),
        max_neighbours,
    )
    return {
