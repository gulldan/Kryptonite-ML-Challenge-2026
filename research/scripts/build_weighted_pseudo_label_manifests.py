"""Build confidence-weighted public pseudo-label manifests from a teacher graph."""
...
def main() -> None:
    ...
    graph_support = _same_cluster_counts(
        labels=labels,
        indices=teacher_indices[:, : args.graph_support_top_k],
    )
    reciprocal_support = _reciprocal_same_cluster_counts(
        labels=labels,
        indices=teacher_indices[:, : args.reciprocal_support_top_k],
    )
    ...
    confidence = _combined_confidence(
        ...
        graph_support_top_k=args.graph_support_top_k,
        reciprocal_support_top_k=args.reciprocal_support_top_k,
        confirmation_source_count=len(confirmation_meta),
    )
    confidence_threshold = max(
        args.min_confidence,
        float(np.quantile(confidence, args.confidence_quantile)),
    )
    verified = (
        (graph_support >= args.min_graph_support)
        & (reciprocal_support >= args.min_reciprocal_support)
        & (confirmation_counts >= args.min_confirming_models)
    )
    size_ok = (cluster_sizes[labels] >= args.min_pseudo_cluster_size) & (
        cluster_sizes[labels] <= args.max_pseudo_cluster_size
    )
    selected_by_confidence = confidence >= confidence_threshold
    selected = size_ok & (selected_by_confidence | verified)
    sample_weights = args.min_sample_weight + (
        (args.max_sample_weight - args.min_sample_weight) * confidence
    )
    sample_weights = np.clip(sample_weights, args.min_sample_weight, args.max_sample_weight)
    ...
    summary: dict[str, Any] = {
        ...
        "selected_by_confidence_count": int((selected & selected_by_confidence).sum()),
        "selected_by_verification_count": int((selected & verified).sum()),
        "pseudo_weight_mean": float(sample_weights[selected].mean()) if selected.any() else None,
        "pseudo_weight_p10": _selected_quantile(sample_weights, selected, 0.10),
        "pseudo_weight_p50": _selected_quantile(sample_weights, selected, 0.50),
        "pseudo_weight_p90": _selected_quantile(sample_weights, selected, 0.90),
...
def _combined_confidence(
    *,
    ...
    graph_support_top_k: int,
    reciprocal_support_top_k: int,
    confirmation_source_count: int,
) -> np.ndarray:
    margin_score = _robust_minmax(margins)
    graph_score = np.clip(graph_support / max(graph_support_top_k, 1), 0.0, 1.0)
    reciprocal_score = np.clip(
        reciprocal_support / max(reciprocal_support_top_k, 1),
        0.0,
        1.0,
    )
    if confirmation_source_count > 0:
        confirmation_score = np.clip(confirmation_counts / confirmation_source_count, 0.0, 1.0)
    else:
        confirmation_score = np.zeros_like(margin_score)
    return np.clip(
        (0.40 * margin_score)
        + (0.30 * graph_score)
...
def _reciprocal_same_cluster_counts(*, labels: np.ndarray, indices: np.ndarray) -> np.ndarray:
    top_sets = [set(map(int, row)) for row in indices]
    counts = np.zeros(indices.shape[0], dtype=np.int16)
    for row_index, row in enumerate(indices):
        label = labels[row_index]
        count = 0
        for neighbor in row:
            neighbor_index = int(neighbor)
            if labels[neighbor_index] == label and row_index in top_sets[neighbor_index]:
                count += 1
...
def _write_pseudo_manifest(
    *,
    ...
    selected: np.ndarray,
    selected_by_confidence: np.ndarray,
    verified: np.ndarray,
    label_prefix: str,
    dataset_name: str,
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        for row_index in np.flatnonzero(selected):
            ...
            selected_by = "confidence" if selected_by_confidence[row_index] else "verification"
            payload = {
                "schema_version": "kryptonite.manifest.v1",
                "record_type": "utterance",
                "dataset": dataset_name,
                "source_dataset": "test_public_weighted_pseudo_labels",
                ...
                "channel": "mono",
                "pseudo_cluster_id": cluster_id,
                "pseudo_cluster_size": int(cluster_sizes[cluster_id]),
                "pseudo_teacher_margin": round(float(margins[row_index]), 8),
                "pseudo_graph_support": int(graph_support[row_index]),
                "pseudo_reciprocal_support": int(reciprocal_support[row_index]),
                "pseudo_confirmation_count": int(confirmation_counts[row_index]),
                "pseudo_cluster_confidence": round(float(confidence[row_index]), 8),
                "pseudo_sample_weight": round(float(sample_weights[row_index]), 8),
                "pseudo_verified": bool(verified[row_index]),
                "pseudo_selected_by": selected_by,
            }
...
def _write_row_audit(
    *,
    ...
    frame = manifest.select("filepath").with_columns(
        ...
        pl.Series("graph_support", graph_support),
        pl.Series("reciprocal_support", reciprocal_support),
        pl.Series("confirmation_count", confirmation_counts),
        pl.Series("confidence", confidence.astype(np.float32, copy=False)),
        pl.Series("sample_weight", sample_weights.astype(np.float32, copy=False)),
        pl.Series("selected_by_confidence", selected_by_confidence),
        pl.Series("verified", verified),
...
def _selected_quantile(values: np.ndarray, selected: np.ndarray, quantile: float) -> float | None:
    if not selected.any():
        return None
    return float(np.quantile(values[selected], quantile))
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--label-prefix", default="pseudo_ms35_gll_")
    parser.add_argument("--dataset-name", default="participants_ms35_gll_pseudo")
    ...
    parser.add_argument("--cluster-max-size", type=int, default=160)
    ...
    parser.add_argument("--min-pseudo-cluster-size", type=int, default=5)
    parser.add_argument("--max-pseudo-cluster-size", type=int, default=120)
    ...
    parser.add_argument("--graph-support-top-k", type=int, default=20)
    parser.add_argument("--reciprocal-support-top-k", type=int, default=20)
    ...
    parser.add_argument("--min-confirming-models", type=int, default=1)
    parser.add_argument("--min-graph-support", type=int, default=4)
    parser.add_argument("--min-reciprocal-support", type=int, default=1)
    parser.add_argument("--confidence-quantile", type=float, default=0.18)
    parser.add_argument("--min-confidence", type=float, default=0.25)
    parser.add_argument("--min-sample-weight", type=float, default=0.35)
    parser.add_argument("--max-sample-weight", type=float, default=1.0)
