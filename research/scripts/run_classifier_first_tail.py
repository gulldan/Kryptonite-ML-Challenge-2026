...
def main() -> None:
    ...
    if args.class_cache_only:
        rows: dict[str, Any] = {
            ...
            "mode": "class_cache_only",
            **classifier_meta,
        }
        (output_dir / f"{args.experiment_id}_class_cache_summary.json").write_text(
            json.dumps(rows, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        pl.DataFrame([{key: _csv_value(value) for key, value in rows.items()}]).write_csv(
            output_dir / f"{args.experiment_id}_class_cache_summary.csv"
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--class-cache-only", action="store_true")
