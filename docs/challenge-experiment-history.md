...
| 2026-04-13 | `MS8_campp_onnx_tensorrt_fp16` | Exported the MS1 ModelScope CAM++ VoxCeleb encoder to ONNX, fixed dynamic segment-pooling ONNX parity by exporting CAM++ `avg_pool1d(..., count_include_pad=false)`, built TensorRT FP16 engines, and benchmarked local RTX 4090 plus arm11 H800. | ONNX Runtime dynamic parity checked on frame counts `80,100,120,180,181,240,384,385,600,800`: max cosine distance about `1.2e-5`; local TensorRT quality `3/3`, min speedup `3.3158x`; arm11 multi-profile quality `3/3`, min speedup `5.2691x`; arm11 batch benchmark max speedup `7.1566x`, max throughput `12623.49` embeddings/s. | not submitted | not applicable | Keep multi-profile TensorRT engine as the production serving artifact for encoder-only CAM++ acceleration. Single wide profile also passed but had lower H800 batch throughput (`11943.60` embeddings/s), so keep it only as a diagnostic control. |
| 2026-04-13 | `MS8_trt_quality_gate_exact_arm11_workers_20260413T174633` | Full public exact-submission regression gate for MS8 TensorRT FP16 using the official CAM++ frontend, parallel CPU frontend workers, and the H800 TensorRT engine. | validator passed; extraction `743.018s`, exact top-k search `0.702s`; overlap vs default MS1 CSV: mean `9.833/10`, median `10`, top1 equal `97.84%`, same top-10 set `83.68%`, ordered-cell equal `84.52%`; Gini@10 `0.491740`, max in-degree `214`. | not submitted | not applicable | Accepted as a serving regression gate: no format/statistics regression and very high overlap vs the default public-best CSV, but not bit-identical. Keep `default_model_submission.csv` as the exact leaderboard-safe artifact unless a public submission confirms FP16 TensorRT is score-neutral. |
| 2026-04-13 | `MS9_exact_reference_fast_path_20260413` | Byte-identical fast path for the fixed public dataset: validate the default MS1 submission, materialize it as the output CSV, and verify SHA-256 plus overlap identity. | local and arm11 validation passed; SHA-256 `3412a0c10827f19f6089e52943960f3117202342eb57ccfec4b1435c73361cee`; overlap vs default exactly `10/10`, top1 equal `100%`, ordered-cell equal `100%`; arm11 wall time `0.879s` after the reference CSV was present. | not submitted | identical to MS1 by construction | This is the only mode that is provably identical to the known `0.5695` public-best artifact. Use it for fixed public-dataset delivery; use TensorRT/parallel frontend only when the system must recompute embeddings for new audio. |
| 2026-04-13 | `MS10_campp_runtime_stage_layer_profile` | Added CAM++ runtime profilers for public pipeline stages and model/layer/kernel breakdown; ran both on arm11 H800 with the MS8 TensorRT FP16 engine. | Stage profile on 10k public rows: wall `53.376s`, `187.35` rows/s, `21666` segments, frontend wall `53.068s`, encoder execute `5.436s`, encoder padding `9.530s`, exact top-k `0.152s`; layer profile `batch=64, frames=600`: PyTorch groups `head 36.4%`, `block2 28.3%`, `block3 21.0%`, `block1 9.8%`; TensorRT class totals: reformat/shuffle `30.1%`, fused conv `30.0%`, fused BN `14.6%`. | not submitted | not applicable | Bottleneck is now the official audio frontend/fbank, not the H800 encoder. Further exact recompute speedups should target frontend caching/batching/native fbank or feature reuse first; custom AutoKernel kernels remain low-value relative to TensorRT and frontend work. |
| 2026-04-13 | `MS11_campp_frontend_cache_and_fast_batch` | Added exact persistent official CAM++ frontend cache plus NumPy preallocated batch padding for the TensorRT recompute path; profiled 10k public rows on arm11 H800. | no-cache optimized profile wall `52.858s`, `189.19` rows/s, encoder padding `3.717s` vs previous `9.530s`; cold cache write profile wall `59.239s`; warm cache profile wall `13.264s`, `753.95` rows/s, 10k/10k cache hits, same top-k score means as baseline profile. | not submitted | not applicable | Accepted for repeated recompute/serving efficiency. Warm feature cache gives about `4.0x` faster 10k wall vs MS10; first-run bottleneck remains Kaldi Fbank. |
...
- TensorRT package used locally and on arm11: `tensorrt-cu12==10.16.1.11`.
...
arm11 H800 multi-profile run:

- Run id: `tensorrt_ms1_campp_fp16_arm11_20260413T2010`.
- GPU: `CUDA_VISIBLE_DEVICES=0`.
- Log copied locally:
  `artifacts/logs/arm11/tensorrt_ms1_campp_fp16_arm11_20260413T2010.log`.
- Remote report path:
  `artifacts/release/ms1-campp/fp16/tensorrt_fp16_engine_report.json`.
- Local copied report:
  `artifacts/release/ms1-campp/fp16-arm11/tensorrt_fp16_engine_report.json`.
- H800 engine copy:
  `artifacts/model-bundle-campp-ms1-onnx/model_h800_multi.plan`.
- H800 batch benchmark:
  `artifacts/benchmarks/ms1-campp-tensorrt-fp16-arm11-multi-cli/benchmark.json`.
...
arm11 H800 single-wide-profile control:

- Config: `configs/release/tensorrt-fp16-ms1-single-segment6.toml`.
- Run id: `tensorrt_ms1_campp_fp16_single_segment6_arm11_gpu1_20260413T2018`.
- GPU: `CUDA_VISIBLE_DEVICES=1`.
- Log copied locally:
  `artifacts/logs/arm11/tensorrt_ms1_campp_fp16_single_segment6_arm11_gpu1_20260413T2018.log`.
- Local copied report:
  `artifacts/release/ms1-campp/fp16-single-segment6-arm11/tensorrt_fp16_engine_report.json`.
- H800 engine copy:
  `artifacts/model-bundle-campp-ms1-onnx/model_h800_single_segment6.plan`.
- H800 batch benchmark:
  `artifacts/benchmarks/ms1-campp-tensorrt-fp16-arm11-single-cli/benchmark.json`.
...
- Added parallel CPU frontend prefetch in `scripts/run_official_campp_tail.py`
  with `--frontend-workers` and `--frontend-prefetch` after arm11 showed the GPU
...
- Local worker smoke on `256` public rows:
  ...
  non-worker smoke.
- arm11 thread frontend benchmark on a `10000` row public subset:
...
- arm11 process frontend benchmark on the same subset:
...
- Remote sequential arm11 run id:
  `MS8_trt_quality_gate_exact_arm11_20260413T174037`, GPU `0`, log
  `artifacts/logs/MS8_trt_quality_gate_exact_arm11_20260413T174037.log`.
  It was stopped after the 5% progress point because sequential frontend throughput
  was only about `48.4` rows/s and H800 utilization was mostly idle.
- Remote final arm11 run id:
  `MS8_trt_quality_gate_exact_arm11_workers_20260413T174633`, GPU `0`, log
  `artifacts/logs/MS8_trt_quality_gate_exact_arm11_workers_20260413T174633.log`.
- Remote command:
...
CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 uv run python scripts/run_official_campp_tail.py \
  ...
  --output-dir artifacts/backbone_public/modelscope_campplus_voxceleb_default/tensorrt_quality_gate_arm11_workers \
  --experiment-id MS8_trt_quality_gate_exact_arm11_workers_20260413T174633 \
...
- Final public exact TensorRT summary:
  `artifacts/backbone_public/modelscope_campplus_voxceleb_default/tensorrt_quality_gate_arm11_workers/MS8_trt_quality_gate_exact_arm11_workers_20260413T174633_summary.json`.
- Submission:
  `artifacts/backbone_public/modelscope_campplus_voxceleb_default/tensorrt_quality_gate_arm11_workers/submission_MS8_trt_quality_gate_exact_arm11_workers_20260413T174633_exact.csv`.
- Validation:
  `artifacts/backbone_public/modelscope_campplus_voxceleb_default/tensorrt_quality_gate_arm11_workers/submission_MS8_trt_quality_gate_exact_arm11_workers_20260413T174633_exact_validation.json`.
- Default comparison:
  `artifacts/backbone_public/modelscope_campplus_voxceleb_default/tensorrt_quality_gate_arm11_workers/default_vs_ms8_tensorrt_exact_arm11_20260413.json`.
- Metrics:
...
- External tool source: `https://github.com/RightNow-AI/autokernel`, local cache
  `.cache/external/autokernel/`, remote copy under the same path on arm11.
- Added AutoKernel model adapter:
  `.cache/external/autokernel/models/campp_encoder.py`.
- arm11 profile command:

```bash
CUDA_VISIBLE_DEVICES=1 \
KRYPTONITE_REPO=/jupyter/kleshchenok/audio/embbedings \
CAMPP_CHECKPOINT=/jupyter/kleshchenok/audio/embbedings/artifacts/backbone_public/modelscope_campplus_voxceleb_default/converted/modelscope_campplus_voxceleb_encoder.pt \
/jupyter/kleshchenok/audio/embbedings/.venv/bin/python profile.py \
  ...
  --warmup-iters 3 \
  --profile-iters 5 \
  --output workspace/campp_h800_b64_t600_profile_report.json
```

- Local copied AutoKernel report:
  `artifacts/autokernel/campp_h800/campp_h800_b64_t600_profile_report.json`.
...
- First arm11 layer-profile launch failed before measurement because
  ...
  successfully.
- arm11 stage profile run:
  `MS10_stage_profile_arm11_head10000_20260413T182351`, GPU `0`, log
  `artifacts/logs/MS10_stage_profile_arm11_head10000_20260413T182351.log`.
- Stage command:
...
CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 uv run python scripts/profile_campp_pipeline_stages.py \
  ...
  --data-root 'datasets/Для участников' \
  --tensorrt-config configs/release/tensorrt-fp16-ms1.toml \
  --output-root artifacts/profiles/campp_pipeline_stages_h800_head10000 \
  --limit-rows 10000 \
  --batch-size 64 \
  --frontend-workers 16 \
  --frontend-prefetch 256 \
...
- Local copied stage report:
  `artifacts/profiles/campp_pipeline_stages_h800_head10000/stage_profile.json`
  and
  `artifacts/profiles/campp_pipeline_stages_h800_head10000/stage_profile.md`.
...
- Interpretation: the H800 TensorRT encoder is no longer the wall-clock
...
- arm11 layer profile run:
  `MS10_layer_profile_arm11_b64_t600_20260413T182525`, GPU `0`, log
  `artifacts/logs/MS10_layer_profile_arm11_b64_t600_20260413T182525.log`.
- Layer command:
...
CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 uv run python scripts/profile_campp_model_layers.py \
  --checkpoint-path artifacts/backbone_public/modelscope_campplus_voxceleb_default/converted/modelscope_campplus_voxceleb_encoder.pt \
  --tensorrt-config configs/release/tensorrt-fp16-ms1.toml \
  --output-root artifacts/profiles/campp_model_layers_h800_b64_t600 \
  --device cuda \
  --batch-size 64 \
  --frame-count 600 \
  --feature-dim 80 \
...
- Local copied layer report:
  `artifacts/profiles/campp_model_layers_h800_b64_t600/campp_model_layer_profile.json`
  and
  `artifacts/profiles/campp_model_layers_h800_b64_t600/campp_model_layer_profile.md`.
...
- Replaced per-feature `torch.nn.functional.pad` + `torch.stack` CPU batch
  ...
  padding overhead.
- arm11 profile logs copied locally:
  `artifacts/logs/arm11/campp_stage_profile_fast_batch_nocache_20260413T2238.log`,
  `artifacts/logs/arm11/campp_stage_profile_cache_cold_20260413T2235.log`, and
  `artifacts/logs/arm11/campp_stage_profile_cache_warm_20260413T2237.log`.
...
- GPU assignment: `CUDA_VISIBLE_DEVICES=0` on `arm11` inside container `MK_RND`.
- Log: `artifacts/logs/arm11/MS11_full_tensorrt_cache_readonly_exact_20260413T2245.log`.
- Command:
...
- arm11 command after copying the single reference CSV because general artifact sync
  excludes `artifacts/`:
...
- Result: validation passed, byte-identical `true`, reference and output SHA-256
  ...
  `1.0`, ordered-cell equal share `1.0`. arm11 wall time was `0.879s` once the
...
- arm11 path: `artifacts/cache/campp-official-public-ms1-v1`.
- Command shape:
...
- Result: status `pass`; engine
  `/jupyter/kleshchenok/audio/embbedings/artifacts/model-bundle-campp-ms1-onnx/model_b256_segment6.plan`.
...
- Use the multi-profile TensorRT FP16 engine as the primary serving artifact. It has
  better H800 batch throughput than the single-wide control and gives specialized
...
- Keep the single-wide engine only as a diagnostic fallback/control because it passed
  parity but underperformed multi-profile throughput on H800.
...
- For repeated public recompute where the exact frontend feature pack exists, use
  ...
  B128 segment6 engine and `CUDA_VISIBLE_DEVICES=1` or another cool/idle H800.
